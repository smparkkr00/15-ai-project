...
# 1. 상위 기본 경로 설정 (2_Validation 폴더로 변경)
NOISE_BASE_DIR = r"C:\Users\KDT34\Documents\Data\2_noise_data\2_Validation\1_val_noise_data"
JSON_BASE_DIR = r"C:\Users\KDT34\Documents\Data\2_noise_data\2_Validation\2_val_json_data"
NPY_SAVE_DIR = r"C:\Users\KDT34\Documents\Data\2_noise_data\3_npy\2_Validation\noise"
CSV_SAVE_DIR = r"C:\Users\KDT34\Documents\Data\2_noise_data\3_npy\2_Validation\json"

# 2. 전처리 스펙 설정 (Training과 완벽히 일치해야 함)
TARGET_SR = 16000
DURATION = 8  
TARGET_LENGTH = TARGET_SR * DURATION  # 128,000 샘플
LIMIT_PER_FOLDER = 1500  # 폴더당 최대 처리 개수 (Validation 데이터가 1500개 미만이면 있는 만큼만 처리됩니다)

# 저장 폴더 생성
os.makedirs(NPY_SAVE_DIR, exist_ok=True)
os.makedirs(CSV_SAVE_DIR, exist_ok=True)

# 10개 세부 하위 경로 리스트 (이름 동일)
sub_folders = [
...
for sub_path in sub_folders:
    ...
    print(f"\n📂 [Val - {folder_name}] 폴더에서 {len(json_files)}개의 파일 처리를 시작합니다.")
    
    # 각 폴더 내부 파일 전처리 루프
    for json_path in tqdm(json_files, desc=f"Processing Val {folder_name}"):
        try:
            # [3-1] JSON 로드
...
# 4. 일상소음 통합 검증 정답지 생성 (noise_val_labels.csv)
if meta_records:
    df = pd.DataFrame(meta_records)
    csv_save_path = os.path.join(CSV_SAVE_DIR, "noise_val_labels.csv")
    df.to_csv(csv_save_path, index=False, encoding='utf-8-sig')
    print(f"\n✨ Validation 일상소음 전처리 완료! 총 {len(meta_records)}개의 npy 파일 빌드 완료.")
